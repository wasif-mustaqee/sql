...
-- director mapping
select count(distinct(movie_id)) as total_rows from director_mapping;
-- result
-- there are total of 3577 rows in the  director_mapping
...
-- result
-- total no of rows in movie column 7997
...
select *from movie;
select title ,count(*)from  movie where title=null;

select year,count(year) from movie where year=null;

select date_published,count(*) from movie where date_published=null;
-- NULL VALUES =0
...
SELECT ratings,count(*) from movie where ratings is null;
select *from movie;


-- alternate method of finding null values in a table 
...
select genre,count(movie_id)as total from genre 
GROUP BY genre
ORDER BY total desc ;
...
with genre_id_count as(
	select movie_id,
		count(genre) as g_count 
	from genre
	GROUP BY movie_id
    having g_count=1)
select count(g_count) AS total 
from genre_id_count;


-- result
-- there are  3289 movies which belong to only one gender
...
-- Type your code below:
with g_count as (
select genre,count(movie_id) as count_genre,
...
-- result 
-- thriller with count of 1484 has a rank of 3
...
with movies_in_1819 as(
select  date_published from movie m
inner join 
ratings r
on m.id=r.movie_id
where median_rating>8 and (year(date_published)=2018 and month(date_published) not in (1,2,3,4)) or (year(date_published)=2019 and month(date_published) in (1,2,3,4))
)
select count(*) from movies_in_1819;
...
select country,sum(total_votes) as total_votes from 
movie m
inner join
ratings r
on 
...
-- result 
-- german movies has as a total of 106710 votes
-- italian movies has a total of 779965 votes 
...
SELECT DISTINCT name AS actor_name, COUNT(n.id) AS movie_count
FROM ratings AS r
...
select production_company prod_comp,
sum(total_votes) total_votes,
RANK() OVER (ORDER BY sum(total_votes)desc) AS PROD_CAMP_RANK
from movie m
inner join
ratings r
ON
...
select names.name as actress_name,
		total_votes,count(movie.id) as movie_count,
...
names
    ...
    where (role_mapping.category like 'actress' and movie.country like 'india' and languages='hindi') 
...
select title,avg_rating,
        ...
        when avg_rating <5 THEN 'flop movies' END
...
select g.genre,
			...
			sum(ROUND(AVG(m.duration))) OVER(ORDER BY genre ROWS UNBOUNDED PRECEDING) as duration_sum,
...
where avg_rating>8 and category='actress'  and g.genre='drama'
group by name
...
-- result of top 3 female actresses based on average ratings
-- 1:Sangeetha Bhat 
-- 2:Fatmire sahiti
-- 3:Adriana Matoshi
